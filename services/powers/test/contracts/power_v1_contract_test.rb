...
# Asserts the *exact* agreed shape of the Power v1 contract (see
# contracts/powers/v1/). This test exists to make an accidental contract
...
class PowerV1ContractTest < ActionDispatch::IntegrationTest
  ...
  test "validation error envelope exposes exactly the documented fields" do
    # This service currently has no validation that can fail (root Power
    # has none — see the Phase 7 compatibility table). This test documents
    ...
    error_json = {
      error: { code: "VALIDATION_ERROR", message: "Power could not be saved.", details: { name: [ "can't be blank" ] } }
    }.as_json
